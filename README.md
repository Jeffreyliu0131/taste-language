...
使用最新的 Node.js 22 补丁版本（至少 22.22.2；本次验证为 22.23.2）。
...
依赖维护（2026-09-21）：`typescript-eslint` 当前要求 TypeScript 低于 6.1，因此暂缓 TypeScript 7 升级，等待上游兼容支持。Dependabot 将 Vitest 与 `@vitest/*` 放入同一升级组，避免测试运行器和覆盖率插件版本冲突；CI 跟随最新 Node.js 22 补丁版本。
